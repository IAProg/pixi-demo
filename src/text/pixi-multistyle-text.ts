/**
 * Forked from https://github.com/FilamentGames/pixi-multistyle-text
...
export interface TextStyleSpriteConfig extends TextStyleExtended {
    src: string;
    width: number;
    height: number;
    yOffset: number;
...
export interface TextStyleSprite extends TextStyleExtended {
    src: Texture;
    width: number;
    height: number;
    yOffset: number;
...
export interface TextStyleSetConfig {
    default: TextStyleExtended
    [key: string]: (TextStyleExtended | TextStyleSpriteConfig);
}

export interface TextStyleSet {
    default: TextStyleExtended
    [key: string]: (TextStyleExtended | TextStyleSprite);
}
...
interface TextWithPrivateMembers {
    ...
    _generateFillStyle(style: object, lines: string[], metrics: PIXI.TextMetrics): string | number | CanvasGradient;
}
...
export default class MultiStyleText extends PIXI.Text {
    ...
    public static isTextStyleSet(textStyle: TextStyle | TextStyleSet): textStyle is TextStyleSet {
        return (textStyle as TextStyleSet).default !== undefined;
    }

    public set styles(styles: TextStyleSet) {
        this.textStyles = {
            default: { ...MultiStyleText.DEFAULT_TAG_STYLE }
        };
        ...
        this.withPrivateMembers()._style = new PIXI.TextStyle(this.textStyles['default'] as any);
        this.withPrivateMembers().dirty = true;
    }

    constructor(text: string, styles: TextStyleSet) {
        super(text);
        // Forced ovewrite of private updateTexture method of PIXI.Text
        this['updateTexture'] = this.updateTextureOW;

        this.styles = styles;
    }

    public setTagStyle(tag: string, style: TextStyleExtended): void {
        if (this.textStyles === undefined ) return;
    ...
    private replaceSpriteText(): string {
        let regexString = '<sprite style=\\w{1,}/>';
        regexString = `(${regexString})`;
        const regex = new RegExp(regexString, 'g');
        const textParts = this.text.split(regex);

        let finalString: string = '';
        textParts.forEach((value: string, index: number) => {
            if (value.match(regexString)) {
                const styleName: string = value.substring(14,value.length - 2);
                finalString += '<' + styleName + '>⊠</' + styleName + '>';
            } else {
                finalString += value;
            }
        });
    ...
    public updateText(): void {
        ...
        const spriteParsedText = this.replaceSpriteText();

        this.hitboxes = [];

        this.texture.baseTexture.resolution = this.resolution;
        let textStyles = this.textStyles || {} as { [key: string]: string };
        let outputText = spriteParsedText;

        if (this.withPrivateMembers()._style.wordWrap) {
            outputText = this.wordWrap(spriteParsedText);
        }
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            for (let j = 0; j < outputTextData[i].length; j++) {
                ...
                if (Reflect.has(sty,'src')) {

                    const spriteStyle = (sty as TextStyleSprite);
                    outputTextData[i][j].width = spriteStyle.width;
                    ...
                    lineYMax = Math.max(
                        lineYMax,
                        spriteStyle.height + spriteStyle.yOffset);
                } else {
                    this.context.font = this.getFontString(sty);
        ...
        for (let i = 0; i < outputTextData.length; i++) {
            ...
            for (let j = 0; j < line.length; j++) {
                ...
                if (Reflect.has(style, 'src')) {
                    ...
                    linePositionX += (style as any).width;
                }
        ...
        drawingData.forEach(({ style, text, x, y, width }) => {
            if (Reflect.has(style, 'src')) {
                ...
                if (source)
                    ...
                    x,
                    y - spriteStyle.height - spriteStyle.yOffset,
                    spriteStyle.width,
                    spriteStyle.height);
            } else {
                if (!style.dropShadow) {
                ...
                const rgb = PIXI.utils.hex2rgb(typeof dropFillStyle === 'number' ? dropFillStyle : PIXI.utils.string2hex(dropFillStyle!));
                this.context.shadowColor = `rgba(${rgb[0] * 255},${rgb[1] * 255},${rgb[2] * 255},${style.dropShadowAlpha})`;
        ...
        drawingData.forEach(({ style, text, x, y, width, ascent, descent, tag }) => {
            if (Reflect.has(style, 'src')) {
                ...
                const generatedStyle = new PIXI.TextStyle(style);
                const measured = PIXI.TextMetrics.measureText(text || ' ', generatedStyle, style.wordWrap, this.canvas);
                this.context.fillStyle = ((this as unknown) as TextWithPrivateMembers)._generateFillStyle(generatedStyle, [text], measured) as string | CanvasGradient;
    ...
    private _getTextDataPerLine(lines: string[]) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (matches.length === 0) {
                lineTextData.push(this.createTextData(lines[i], styleStack[styleStack.length - 1], tagStack[tagStack.length - 1]) as TextData);
            }
            else {
                ...
                for (let j = 0; j < matches.length; j++) {
                    ...
                    if (matches[j][0][1] === "/") { // reset the style if end of tag
                        ...
                        if (tagStyle === TAG_STYLE.bbcode && matches[j][0].includes('=') && this.textStyles[matches[j][1]]) {
                            ...
                            const bbStyle: { [key: string]: string } = {};
                ...
                if (currentSearchIdx < lines[i].length) {
                    ...
                    lineTextData.push(result as TextData);
                }
            }
    ...
    private getFontString(style: TextStyleExtended): string {
        return new PIXI.TextStyle(style as any).toFontString();
    }

    private createTextData(text: string, style: TextStyleExtended, tag: TagData): Partial<TextData> {
        return {
            text,
            style,
            width: 0,
